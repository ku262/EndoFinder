...
class RotateTransform(ClassyTransform):
    ...
    def __call__(self, image: Image.Image, target: Image.Image):
        ...
        target_rotated = target.rotate(
            angle=self._angle_deg_sampler(), resample=Image.BILINEAR
        )
        return image_rotated, target_rotated
...
class MaybeApplyTransform(ClassyTransform):
    ...
    def __call__(self, image: Image.Image, target: Image.Image) -> Image.Image:
...
class ResizeLongEdge(ClassyTransform):
    ...
    def __call__(self, image: Image.Image, target: Image.Image) -> Image.Image:
        ...
        if h > w:
            ...
            w = self.size
            h = int(scale * h + 0.5)
        return transforms.Resize((w, h))(image), transforms.Resize((w, h))(target)
