...
from EndoFinder.lib.inference import Inference
from EndoFinder.datasets.polyp import POLYPEvalDataset
from EndoFinder.lib.util import parse_bool
...
polyp_parser.add_argument("--k", default=10, type=int)
polyp_parser.add_argument(
    "--global_candidates",
    default=False,
    type=parse_bool,
...
polyp_parser.add_argument("--metadata", help="Metadata column to put in the result CSV")
...
logger = logging.getLogger("polyp_eval.py")
...
def make_validation_dataset(path, include_train=False, size=288, preserve_aspect_ratio=False):
    ...
    return POLYPEvalDataset(path, transform=transforms, include_train=include_train)
...
def evaluate_all(dataset, outputs, codecs_arg, score_norm_arg, **kwargs):
    ...
    queries = dataset_split(outputs, POLYPEvalDataset.SPLIT_QUERY)
    refs = dataset_split(outputs, POLYPEvalDataset.SPLIT_REF)
    training = dataset_split(outputs, POLYPEvalDataset.SPLIT_TRAIN)
    logger.info(
...
def evaluate(
    ...
    **kwargs,
    ...
    try:
        ...
        logger.error(f"POLYP eval {codec}: {e}")
        return None
    ...
    logger.info(
        f"POLYP eval ({score_norm or 'no norm'}, {codec}): {json.dumps(metrics)}"
    )
    return metrics
...
def main(args):
    ...
    csv_filename = os.path.join(args.output_path, "polyp_metrics.csv")
    df.to_csv(csv_filename, index=False)
    with open(csv_filename, "r") as f:
        logger.info("POLYP metrics:\n%s", f.read())
