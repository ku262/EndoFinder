...
from EndoFinder.lib.distributed_util import cross_gpu_batch

class MaskEmbed(nn.Module):
    """ Mask to Mask Embedding
    """
    def __init__(self, img_size=224, patch_size=16, in_chans=1, embed_dim=1):
        super().__init__()
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
        ...
        self.target_embed = MaskEmbed(img_size, patch_size, 1, 1)
    ...
    def assign_masking(self, x, target, mask_ratio):
        """
        According to the target, preserve the polyp portion
        x: [N, L, D], sequence
        """
        N, L, D = x.shape
        len_keep = int(L * (1 - mask_ratio))
        ...
        random_values = torch.rand(target.size()) * 0.99 + 0.01
        random_values = random_values.to(target)

        target[zero_indices] = random_values[zero_indices]

        # sort noise for each sample
        ids_shuffle = torch.argsort(target, dim=1, descending=True)  # ascend: large is keep, small is remove
    ...
    def forward_encoder(self, x, target, mask_ratio):
        # embed patches
        ...
        x, mask, ids_restore = self.assign_masking(x, target, mask_ratio)
        # append cls token
    ...
    def forward_sscd_loss(self, latent, instance_labels, mixup=False, entropy_weight=5, infonce_temperature=0.05):

        cls_token = latent[:, 0]
        cls_token = self.head(cls_token)
        embeddings = self.embeddings(cls_token)

        similarity, match_matrix, identity = self.cross_gpu_similarity(
            embeddings, instance_labels, mixup
        )
